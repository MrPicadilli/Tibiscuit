    ...
    void DrawVisionCone()//this method creates the vision cone mesh
    {
        ...
        for (int i = 0; i < VisionConeResolution; i++)
        {
            ...
            if (Physics.Raycast(transform.position, RaycastDirection, out RaycastHit hit, VisionRange, VisionObstructingLayer))
            {
                ...
                if(!transform.parent.GetComponent<AgentController>().hasSeenPlayer && hit.collider.gameObject.layer == LayerMask.NameToLayer("Player")){                   
                    transform.parent.GetComponent<AgentController>().PlayerFound(hit.collider.gameObject.transform.position);
                }
